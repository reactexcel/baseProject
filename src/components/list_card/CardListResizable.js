...
const CardList = props => {
  ...
  const layout = [
    {
      i: `list-${props.index}`,
      x: (props.item && props.item.x) || 0,
      y: (props.item && props.item.y) || 0,
      w:
        props.item && props.item.w
          ? Math.ceil(props.item.w)
          : Math.ceil(240 / 10),
      h: Math.ceil(props.item.h / 30),
      isResizable: true,
      isDraggable: true
    }
  ];
  ...
  return (
    ...
    layout={layout}
    onLayoutChange={(event)=> changeSizeApi(event)}
  >
    <DIV className="cardData">
